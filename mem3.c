/******************************************************************************** 
Memory Allocator : Program to manage its heap memory and allocate and free memory as per user requirements without making use of C built in functions ‘malloc()’ and ‘free()’.  Every time a call is made for allocation of memory, the program allocates memory and displays the available memory after calculating the size used by headers that maintain memory address and size information.  Similarly, when a request is made to free the memory, memory is freed, coalesced  and available memory is displayed after taking into account the size used by headers.   *********************************************************************************************/
